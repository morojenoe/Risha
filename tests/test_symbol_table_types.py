import unittest
import src.ast_visitors.semantic_analysis.symbol_table_types as types


class VariableTestCase(unittest.TestCase):
    def test_simple_variable(self):
        variable = types.Variable('current_maximum',
                                  types.VariableType(None, None, None, 'int'),
                                  None)
        self.assertEqual('int current_maximum', str(variable))

    def test_const_variable(self):
        variable = types.Variable('N_MAX',
                                  types.VariableType('const', None, None,
                                                     'double'),
                                  None)
    ...
    def test_reference_qualifier(self):
        variable = types.Variable('count_numbers',
                                  types.VariableType(None, None, 'ref', 'bint'),
                                  None)

        self.assertEqual('ref bint count_numbers', str(variable))

    def test_all_qualifiers(self):
        variable = types.Variable('unique_numbers',
                                  types.VariableType('const', 'static', None,
                                                     'set'),
                                  None)
